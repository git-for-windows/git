...
`worktree.copyOnWrite`::
	When creating a linked worktree, use copy-on-write file copies when
	supported by the platform and file system. Setting this to `false`
	disables the optimization. Defaults to `true`.
